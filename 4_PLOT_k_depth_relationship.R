...
mean.beta <- exp(out2$BUGSoutput$mean$BB[,2] )

# Fake data to predict
fake1 <- seq(min(covs$zdepth), max(covs$zdepth), length=50)
...
est.lineA <- matrix(NA, ncol=length(fake1), nrow=out2$BUGSoutput$n.sims) #container for predicted values


for(i in 1:out2$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineA[i,t] <- exp(out2$BUGSoutput$sims.list$mu.k[i] + out2$BUGSoutput$sims.list$gammaKb1[i] * fake1[t] )
  }
}
...
for(i in 1:length(mean.beta)) { 
  u.alpha[i] <- exp(quantile(out2$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) ) )
  l.alpha[i] <- exp(quantile(out2$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) ) )
}

###########################################
####### FIGURE WITH CRI's
res <- 6
name_figure <- "WE_K_depth.png"
...
x.label <- 'Lake maximum depth (standardized)'
...
plot(mean.beta ~ covs$zdepth,pch=16,axes=F, xlab='',ylab='',cex=0.8,type='n',ylim=c(min(l.alpha), max(u.alpha)) )
...
points(covs$zdepth, mean.beta,pch=16,cex=0.8)

segments(x0=covs$zdepth, x1=covs$zdepth,
