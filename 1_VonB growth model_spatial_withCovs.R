...
# Read in lake covariate data
covs <- read.csv('LakeList.csv')
head(covs)

covs <- covs[order(covs$FISHERIES_WATERBODY_ID),]

# Keep subset of lakes with data
covs <- covs[covs$FISHERIES_WATERBODY_ID %in% dat$FISHERIES_WATERBODY_ID,]
dim(covs)

head(covs)

# log-transform and standardize covariates
covs$depth <- log(covs$MAX_DEPTH_FEET)
covs$zdepth <- as.numeric(scale(covs$depth))
covs$area <- log(covs$LAKE_AREA_GIS_ACRES)
covs$zarea <- as.numeric(scale(covs$area))


# unique(dat$FISHERIES_WATERBODY_ID)

# Remove one lake with no cov data
dat <- dat[!dat$FISHERIES_WATERBODY_ID==6000003,]
length(unique(dat$FISHERIES_WATERBODY_ID))

# Remove lake with missing lake area
head(covs)
covs <- covs[!is.na(covs$zarea),]
dim(covs)

# Now remove that lake with missing data from dat
dat <- dat[dat$FISHERIES_WATERBODY_ID %in% covs$FISHERIES_WATERBODY_ID,]
dim(dat)
length(unique(dat$FISHERIES_WATERBODY_ID))
...
for(j in 1:J){
  Linf[j] <- exp(BB[j,1])
	...
	BB.hat[j,1] <- mu.Linf + gammaLb1 * z1[j] + gammaLb2 * z2[j]
	BB.hat[j,2] <- mu.k + gammaKb1 * z1[j] + gammaKb2 * z2[j]
	BB.hat[j,3] <- mu.t0
}
  ...
  gammaLb1 ~ dnorm(0, 0.0001)
  gammaLb2 ~ dnorm(0, 0.0001)

  gammaKb1 ~ dnorm(0, 0.0001)
  gammaKb2 ~ dnorm(0, 0.0001)
...
params1 <- c("mu.Linf", "mu.k", "mu.t0", "sigma.y","BB","Sigma.B","rho.B",
             "gammaLb1","gammaLb2","gammaKb1","gammaKb2")
...
out2 <- jags(data = data, inits = inits, parameters.to.save = params1, 
            model.file = "vonBmodel2.txt", n.chains = nc, n.thin = nt, n.iter = ni, 
            n.burnin = nb)
...
quantile(out2$BUGSoutput$sims.list$gammaKb1, c(0.05, 0.90))
quantile(out2$BUGSoutput$sims.list$gammaKb2, c(0.05, 0.90))

quantile(out2$BUGSoutput$sims.list$gammaLb1, c(0.05, 0.90))
quantile(out2$BUGSoutput$sims.list$gammaLb2, c(0.05, 0.90))
